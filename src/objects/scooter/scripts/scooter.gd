...
@export var _brake_force: float = 2
@export var _stabilizing_factor: float = 10
...
## Minimal speed required to switch
## angular velocity Z axis lock to
## aligner script.
@export var _alligner_min_speed: float = 10
...
@onready var _pid: PIDController = $PIDController
...
func _integrate_forces(state: PhysicsDirectBodyState3D) -> void:
	...
	if not is_zero_approx(tilt):
		var correction = _pid.update(rad_to_deg(tilt), 0, state.step)
		state.apply_torque(global_basis.z * deg_to_rad(correction) * _stabilizing_factor * mass)
